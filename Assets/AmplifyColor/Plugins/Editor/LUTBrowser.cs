	...
	public class LUTBrowser : EditorWindow
	{
		...
		private bool DrawLibrary( float previewSize, bool showSource )
		{
			...
			if ( sourceCamera != null )
			{
				...
				bool hdr = sourceCamera.allowHDR;
			#else
				bool hdr = sourceCamera.hdr;
			#endif
				RenderTextureReadWrite rw = linearPath ? RenderTextureReadWrite.Linear : RenderTextureReadWrite.sRGB;
				RenderTextureFormat fmt = hdr ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.ARGB32;
				...
				if ( sourceCamera.transform.parent != null )
				{
					clonedCamera.transform.parent = sourceCamera.transform.parent;
					clonedCamera.transform.localPosition = sourceCamera.transform.localPosition;
					clonedCamera.transform.localRotation = sourceCamera.transform.localRotation;
					clonedCamera.transform.localScale = sourceCamera.transform.localScale;
				}
		...
		private void OnGUI()
		{
			...
			GUILayout.BeginHorizontal( EditorStyles.toolbar );
			{
				...
				m_previewCameras = GameObject.FindObjectsByType<Camera>( FindObjectsSortMode.None );
			#else
